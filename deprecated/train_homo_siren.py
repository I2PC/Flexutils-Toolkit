...
def train(outPath, md_file, batch_size, shuffle, step, splitTrain, epochs, cost,
          ...
          refString="p,m", multires=(2, 4, 8), gpu=None, jit_compile=True, tensorboard=True):
    ...
    try:
        def fitModel(args):
            (workflow_step, idx, out, jit_compile, tensorboard) = args
        ...
        for workflow_step in workflow:
            ...
            p.restart()
            p.pipe(fitModel, args=(workflow_step, idx, outPath, jit_compile, tensorboard))
            p.close()
            p.join()
